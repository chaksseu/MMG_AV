    ...
    try:
        with open(prompt_file, newline='', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                ...
                caption = row.get('compressed_video_caption', '').strip()
                if caption:
...
def find_id_by_prompt(csv_path, prompt):
    ...
    match = df[df['compressed_video_caption'] == prompt]
    ...
    try:        
        ...
        for batch_idx in range(num_batches):
            ...
            for i, prompt in enumerate(current_prompts):
                ...
                sample_id = find_id_by_prompt(prompt_file, prompt) # prompt를 보고 csv파일에서 new_caption에 해당 값을 찾아 같은 행의 id열의 값을 사용
...
, help="Video 모델 체크포인트 경로")
    ...
    datasets = ["openvid", "vggsound_sparse"]
    model_type = "FULL" # "LORA" "FULL"

    video_lora_ckpt_path_list = ["checkpoint-step-0", "checkpoint-step-8192", "checkpoint-step-16384", "checkpoint-step-24576", "checkpoint-step-32768"]

    for dataset in datasets:
        video_lora_ckpt = f"/home/work/kby_hgh/VIDEO_FULL_CHECKPOINT_0626/1e-5"
        save_dir = f"/home/work/kby_hgh/0627_video_teacher_{model_type}_{dataset}_inference"
        # save_dir = f"/home/work/kby_hgh/0603_video_teacher_full_{dataset}_inference"
        ...
        for i in range(len(video_lora_ckpt_path_list)):
            ...
            save_path = save_dir + "/" + video_lora_ckpt_path_list[i]
