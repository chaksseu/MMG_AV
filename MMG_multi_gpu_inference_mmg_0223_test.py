...
def parse_args():
    """
    스크립트 실행 시 사용할 ArgumentParser 정의.
    """
    parser = argparse.ArgumentParser(description="Cross-modal Audio-Video Diffusion Inference")

    # 공통 파라미터
    parser.add_argument("--seed", type=int, default=42, help="Random seed (기본값: 0)")
    parser.add_argument("--duration", type=float, default=3.2, help="생성할 오디오 길이(초 단위)")
    parser.add_argument("--fps", type=float, default=12.5, help="비디오 FPS")
    parser.add_argument("--height", type=int, default=320, help="생성할 비디오 높이(px)")
    parser.add_argument("--width", type=int, default=512, help="생성할 비디오 너비(px)")
    parser.add_argument("--frames", type=int, default=40, help="생성할 비디오 총 프레임 수 (기본 -1이면 VideoCrafter의 기본값 사용)")

    # Inference 관련 스케줄링/하이퍼파라미터
    parser.add_argument("--num_inference_steps", type=int, default=25, help="Diffusion 스텝 수")
    parser.add_argument("--audio_ddim_eta", type=float, default=0.0, help="Audio 브랜치용 DDIM ETA 값")
    parser.add_argument("--video_ddim_eta", type=float, default=0.0, help="Video 브랜치용 DDIM ETA 값")
    parser.add_argument("--audio_guidance_scale", type=float, default=7.5, help="Audio classifier-free guidance 스케일")
    parser.add_argument("--video_unconditional_guidance_scale", type=float, default=12.0, help="Video uncond guidance 스케일")
    parser.add_argument("--inference_batch_size", type=int, default=4, help="생성 시 배치 크기")
    ...
    parser.add_argument('--ckpt_dir', type=str, default="/workspace/MMG_CHECKPOINT/checkpint_0224/checkpoint-step-20720/model.safetensors", help='Directory where outputs will be saved.')
    ...
    parser.add_argument("--eval_id", type=str, default="test_run"), 
...
def main():
    """
    스크립트 실행의 메인 함수. 
    분산(Accelerate) 환경에서 prompt를 나눠받아 Inference를 수행하고 결과를 저장한다.
    """
    parser = parse_args()
    ...
    # 체크포인트(혹은 safetensors) 파일 경로
    ckpt_dir = args.ckpt_dir

    # CSV 에서 전체 프롬프트 불러오기
    all_prompts = load_prompts(target_csv_files)  # scripts/evaluation/funcs 에 정의된 예시 함수
    # 원하는 만큼 슬라이싱
    # all_prompts = all_prompts[:10]  # 예: 10개만 쓰고 싶으면
    ...
    accelerator.wait_for_everyone()
    if accelerator.is_main_process:
        print(f"\n[Main] All processes finished. Results saved under: {inference_save_path}")
