...
def find_id_by_prompt(csv_path, prompt):
    df = pd.read_csv(csv_path)
    
    # prompt에 해당하는 new_caption을 가진 행 찾기
    match = df[df['new_caption'] == prompt]
    
    if match.empty:
        match = df[df['caption'] == prompt]
        if match.empty:
            print("csv_path", csv_path)
            print("prompt", prompt)
            return None  # 일치하는 값이 없을 경우

    id_value = match.iloc[0]['id']
    
    # 확장자 제거
    if isinstance(id_value, str) and id_value.endswith('.mp4'):
        id_value = id_value[:-4]
    if isinstance(id_value, str) and id_value.endswith('.wav'):
        id_value = id_value[:-4]
    if isinstance(id_value, str) and id_value.endswith('.flac'):
        id_value = id_value[:-5]
...
def run_inference(
    ...
    try:
        ...
        for batch_idx in range(num_batches):
            ...
            audio_prompts, video_prompts = zip(*current_prompts)



            # 튜플 → 리스트
            audio_prompts_list = list(audio_prompts)
            video_prompts_list = list(video_prompts)
            if accelerator.is_main_process:
                print("#########################################")
                print("audio_prompts: ")
                print(audio_prompts_list)
                print("#########################################")
                print("video_prompts: ")
                print(video_prompts_list)
            ...
            video_text_emb = video_pipeline.get_learned_conditioning(video_prompts_list) ###
            ...
            for i, prompt in enumerate(audio_prompts_list):
                # safe_prompt = sanitize_filename(prompt)
                sample_index = start_idx + i

                sample_id = find_id_by_prompt(target_csv_files, prompt) # prompt를 보고 csv파일에서 new_caption에 해당 값을 찾아 같은 행의 id열의 값을 사용

                base_filename = f"{sample_id}_batch_{sample_index}_proc_{accelerator.process_index}"
...
def main():
    ...
    # dataset_list = ["vgg", "ac", "vbench"]


    # checkpoint_dir_list = ["6047", "12095", "14111", "18143"] 
    ...
    for checkpoint_dir in checkpoint_dir_list:
        for dataset in dataset_list:
            ...
            # vbench
            if dataset == "openvid":
                args.prompt_file = "/home/work/kby_hgh/MMG_01/video_lora_training/0626_processed_Openvid_split_test_with_audio_caption_filtered_summerize.csv"
            if dataset == "audiocaps":
                args.prompt_file = "/home/work/kby_hgh/MMG_AC_test_dataset/0627_one_cap_AC_split_test_with_video_caption.csv"
            ...
            # ckpt_dir = f"/home/work/kby_hgh/MMG_CHECKPOINT/checkpint_tensorboard/0503_1e-4_MMG_OURS/checkpoint-step-{checkpoint_dir}"
            # eval_id = f"NEW_MMG_OURS_{dataset}_step_{checkpoint_dir}"
            
            # ckpt_dir = f"/home/work/kby_hgh/MMG_CHECKPOINT/checkpint_tensorboard/0503_MMG_NAIVE_DISTILL_1e-4_ta_tv_weight_1_1/checkpoint-step-{checkpoint_dir}"
            # eval_id = f"NEW_MMG_NAIVE_DISTILL_{dataset}_step_{checkpoint_dir}"
            

            # ckpt_dir = f"/home/work/kby_hgh/MMG_CHECKPOINT/checkpint_tensorboard/0509_1118_MMG_ONLY_RC_DISTILL_1e-4_ta_tv_weight_1_1/checkpoint-step-{checkpoint_dir}"
            # eval_id = f"MMG_ONLY_RC_DISTILL_{dataset}_step_{checkpoint_dir}"

            # ckpt_dir = f"/home/work/kby_hgh/MMG_CHECKPOINT/checkpint_tensorboard/0511_MMG_ONLY_RC_DISTILL_FILTERED_1e-4_ta_tv_weight_1_1/checkpoint-step-{checkpoint_dir}"
            # eval_id = f"MMG_FILTERED_ONLY_RC_DISTILL_{dataset}_step_{checkpoint_dir}"


            # ckpt_dir = f"/home/work/kby_hgh/MMG_CHECKPOINT/checkpint_tensorboard/0513_FINAL_ALL_MMG_OURS_1e-4_MMG_OURS_1_2/checkpoint-step-{checkpoint_dir}"
            # eval_id = f"FINAL_ALL_MMG_OURS_step_{checkpoint_dir}_{dataset}"

            # ckpt_dir = f"/home/work/kby_hgh/MMG_CHECKPOINT/checkpint_tensorboard/0513_FINAL_FILTERED_MMG_OURS_1e-4_MMG_FILTERED_OURS_1_2/checkpoint-step-{checkpoint_dir}"
            # eval_id = f"FINAL_FILTERED_MMG_OURS_step_{checkpoint_dir}_{dataset}"
            
            # ckpt_dir = f"/home/work/kby_hgh/MMG_CHECKPOINT/checkpint_tensorboard/0514_FINAL_FILTERED_NAIVE_DISTILLATION_1e-4_ta_tv_weight_1_2/checkpoint-step-{checkpoint_dir}"
            # eval_id = f"MMG_FINAL_NAIVE_DISTILL_step_{checkpoint_dir}_{dataset}"

            # ckpt_dir = f"/home/work/kby_hgh/MMG_CHECKPOINT/checkpint_tensorboard/0513_FINAL_FILTERED_MMG_RC_1e-4_ta_tv_weight_1_2/checkpoint-step-{checkpoint_dir}"
            # eval_id = f"MMG_FINAL_FILTERED_RC_DISTILL_{dataset}_step_{checkpoint_dir}"

            # ckpt_dir = f"/home/work/kby_hgh/MMG_CHECKPOINT/checkpint_tensorboard/0524_FINAL_FILTERED_MMG_RC_LENEAR_T_1e-4_ta_tv_weight_1_2_/checkpoint-step-{checkpoint_dir}"
            # eval_id = f"MMG_FINAL_FILTERED_RC_DISTILL_LINEAR_T_{dataset}_step_{checkpoint_dir}"

            # ckpt_dir = f"/home/work/kby_hgh/MMG_CHECKPOINT/checkpint_tensorboard/test_0608_ALL_FULL_MMG_OURS_1e-4_ta_tv_weight_1_2_/checkpoint-step-{checkpoint_dir}"
            ...
            run_inference(
                ...
                target_csv_files=args.prompt_file
            )
