...
# from accelerate import Accelerator  # 제거됨
...
def get_parser() -> argparse.ArgumentParser:
    ...
    # parser.add_argument("--num_gpus", type=int, default=1, help="Number of GPUs (handled by accelerate)")  # 제거됨
    return parser
...
def run_inference(
    ...
    try:
        ...
        video_pipeline = instantiate_from_config(model_config).to(device, dtype)
        ...
        # Setup progress bar
        pbar = tqdm(total=num_batches, desc="Generating", disable=False)
        ...
        for batch_idx in range(num_batches):
            ...
            # Initialize latents
            video_latents = torch.randn(video_noise_shape, device=device, generator=generator, dtype=dtype)
            ...
            for i, prompt in enumerate(current_prompts):
                ...
                base_filename = f"{safe_prompt}_batch_{sample_index}"
        ...
        print(f"Completed inference. Results saved in {args.savedir}.")
...
def main():
    ...
    # Device and dtype 설정
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    dtype = torch.bfloat16 if torch.cuda.is_available() else torch.float32  # bfloat16은 GPU에서만 사용 가능

    # Inference 실행
    run_inference(args, all_prompts, dtype)
