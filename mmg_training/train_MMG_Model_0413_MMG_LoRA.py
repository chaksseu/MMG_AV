...
from dataset_mmg_0413 import AudioVideoDataset
...
def parse_args():
    ...
    parser.add_argument('--panda_csv_path', type=str, default='/workspace/processed_vggsound_sparse_0218/avsync_test', help='Directory where outputs will be saved.')
    parser.add_argument('--panda_gt_test_path', type=str, default='/workspace/processed_vggsound_sparse_0218/avsync_gt_test.csv', help='Directory where outputs will be saved.')
    parser.add_argument('--clotho_csv_path', type=str, default='/workspace/processed_vggsound_sparse_0218/avsync_test', help='Directory where outputs will be saved.')
    parser.add_argument('--clotho_gt_test_path', type=str, default='/workspace/processed_vggsound_sparse_0218/avsync_gt_test.csv', help='Directory where outputs will be saved.')
        ...
        with tqdm(dataloader, desc=f"Epoch {epoch+1}/{args.num_epochs}", unit="batch") as tepoch:
            ...
            for batch_idx, batch in enumerate(tepoch):
                ...
                if (global_step+1) % (args.eval_every * args.gradient_accumulation) == 0:
                    ...
                    if global_step > 20000:
                        _, _, panda_fvd, panda_clip_avg, _, _ = evaluate_model(
                            args=args,
                            accelerator=accelerator,
                            target_csv_files=args.panda_csv_path,
                            target_path=args.panda_gt_test_path,
                            eval_id=f"{args.tensorboard_log_dir}_step_{global_step}_panda",
                            ckpt_dir=safetensor_path,
                            modality="video"
                        )
                    # if accelerator.is_main_process:
                    #     wandb.log({
                    #         "eval/panda_fvd": panda_fvd,
                    #         "eval/panda_clip_avg": panda_clip_avg,
                    #         "step": global_step
                    #     })
                    # if accelerator.is_main_process and writer is not None:
                    #     writer.add_scalar("eval/panda_fvd", panda_fvd, global_step)
                    #     writer.add_scalar("eval/panda_clip_avg", panda_clip_avg, global_step)
                    #     writer.add_scalar("eval/vgg_fvd", vgg_fvd['final'], global_step)

                    clotho_fad, clotho_clap_avg, _, _, _, _ = evaluate_model(
                        args=args,
                        accelerator=accelerator,
                        target_csv_files=args.clotho_csv_path,
                        target_path=args.clotho_gt_test_path,
                        eval_id=f"{args.tensorboard_log_dir}_step_{global_step}_clotho",
                        ckpt_dir=safetensor_path,
                        modality="audio"
                    )
                    # if accelerator.is_main_process:
                    #     wandb.log({
                    #         "eval/clotho_fad": clotho_fad,
                    #         "eval/clotho_clap_avg": clotho_clap_avg,
                    #         "step": global_step
                    #     })
                    # if accelerator.is_main_process and writer is not None:
                    #     writer.add_scalar("eval/clotho_fad", clotho_fad, global_step)
                    #     writer.add_scalar("eval/clotho_clap_avg", clotho_clap_avg, global_step)
