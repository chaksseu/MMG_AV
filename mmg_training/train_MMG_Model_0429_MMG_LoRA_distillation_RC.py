...
from dataset_mmg_rc_0429 import AudioVideoDataset
        ...
        with tqdm(dataloader, desc=f"Epoch {epoch+1}/{args.num_epochs}", unit="batch") as tepoch:
            ...
            for batch_idx, batch in enumerate(tepoch):
                ...
                with accelerator.accumulate(model):
                    ...
                    # ================================================================
                    # 1) 두 종류 배치를 concat  (B == batch_size)
                    # ================================================================
                    # audio / video latent
                    audio_lat_all  = torch.cat([noised_audio_latents,  ta_noised_audio_latents ], dim=0)  # [2B, Ca, Fa, Ta]
                    video_lat_all  = torch.cat([noised_video_latents,  tv_noised_video_latents], dim=0)   # [2B, Cv, Tv, H, W]

                    # text-condition & timestep
                    audio_embed_all = torch.cat([audio_text_embed, ta_audio_text_embed], dim=0)           # [2B, 77, 768]
                    video_embed_all = torch.cat([video_text_embed, tv_video_text_embed], dim=0)           # [2B, Dv]
                    fps_all         = torch.cat([fps_tensor,       tv_fps_tensor      ], dim=0)           # [2B]
                    t_all           = timesteps.repeat(2)                                                # [2B]

                    # ================================================================
                    # 2) 한번에 forward
                    # ================================================================
                    with accelerator.autocast():
                        audio_out_all, video_out_all = model(
                            audio_latents              = audio_lat_all,
                            audio_timestep             = t_all,
                            audio_encoder_hidden_states= audio_embed_all,
                            video_latents              = video_lat_all,
                            video_timestep             = t_all,
                            video_context              = video_embed_all,
                            video_fps                  = fps_all,
                        )

                    # ================================================================
                    # 3) teacher-UNet도 한번에 forward
                    # ================================================================
                    with torch.no_grad():
                        teach_audio_all = teacher_audio_unet(
                            audio_lat_all, t_all, encoder_hidden_states=audio_embed_all, return_dict=False
                        )[0]                                           # [2B, Ca, Fa, Ta]

                        teach_video_all = teacher_video_unet(
                            video_lat_all, t_all, context=video_embed_all, fps=fps_all
                        )                                              # [2B, Cv, Tv, H, W]

                    # ================================================================
                    # 4) 배치 분리  (앞 B개 = TAV, 뒤 B개 = TA / TV)
                    # ================================================================
                    audio_out, ta_audio_out         = audio_out_all.chunk(2, dim=0)   # 각각 [B, …], [B, …]
                    video_out, tv_video_out         = video_out_all.chunk(2, dim=0)

                    teach_audio, teach_ta_audio     = teach_audio_all.chunk(2, dim=0)
                    teach_video, teach_tv_video     = teach_video_all.chunk(2, dim=0)

                    loss_audio  = args.audio_loss_weight         * F.mse_loss(audio_out,  noise_audio)
                    loss_video  = args.video_loss_weight         * F.mse_loss(video_out,  noise_video)
                    ta_distill_loss_audio = args.ta_audio_loss_weight * F.mse_loss(audio_out,  teach_audio)
                    tv_distill_loss_video = args.tv_video_loss_weight * F.mse_loss(video_out,  teach_video)

                    rc_ta_distill_loss_audio = args.rc_ta_audio_loss_weight * F.mse_loss(ta_audio_out, teach_ta_audio)
                    rc_tv_distill_loss_video = args.rc_tv_video_loss_weight * F.mse_loss(tv_video_out, teach_tv_video)
                    ...
                    loss = loss_audio + loss_video + ta_distill_loss_audio + tv_distill_loss_video + rc_ta_distill_loss_audio + rc_tv_distill_loss_video
                    ...
                    if accelerator.is_main_process:
                        ...
                        if writer is not None:
                            ...
                            writer.add_scalar("train/loss_ta_rc_distill_audio", avg_losses_rc_distill_audio, global_step)
                            writer.add_scalar("train/loss_tv_rc_distill_video", avg_losses_rc_distill_video, global_step)
                        ...
                        logging.info(f"train/loss_ta_rc_distill_audio: {avg_losses_rc_distill_audio}, global_step: {global_step}")
                        logging.info(f"train/loss_tv_rc_distill_video: {avg_losses_rc_distill_video}, global_step: {global_step}")
