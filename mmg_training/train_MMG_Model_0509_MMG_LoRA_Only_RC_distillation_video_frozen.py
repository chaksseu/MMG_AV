...
class CrossModalCoupledUNet(nn.Module):
    ...
    def __init__(self, audio_unet, video_unet, cross_modal_config):
        ...
        print(f"Trainable parameters in audio_unet : {audio_trainable_params}")
        print(f"Trainable parameters in video_unet : {video_trainable_params}")
...
def main():
    ...
    for name, param in audio_unet.named_parameters():
    ...
    # =================================================
    # gpu4_gpu8 = 0
    gpu4_gpu8 = global_step // 2
    global_step += gpu4_gpu8
    ##########################
    ...
    for epoch in range(args.num_epochs):
        with tqdm(dataloader, desc=f"Epoch {epoch+1}/{args.num_epochs}", unit="batch") as tepoch:
            ...
            for batch_idx, batch in enumerate(tepoch):
                ...
                with accelerator.accumulate(model):
                    ...
                    if accelerator.is_main_process:
                        ...
                        if writer is not None:
                            writer.add_scalar("train/loss", avg_losses, global_step+gpu4_gpu8)
                            writer.add_scalar("train/loss_audio", avg_losses_audio, global_step+gpu4_gpu8)
                            writer.add_scalar("train/loss_video", avg_losses_video, global_step+gpu4_gpu8)
                            writer.add_scalar("train/loss_ta_rc_distill_audio", avg_losses_rc_distill_audio, global_step+gpu4_gpu8)
                            writer.add_scalar("train/loss_tv_rc_distill_video", avg_losses_rc_distill_video, global_step+gpu4_gpu8)
                            writer.add_scalar("epoch", epoch, global_step+gpu4_gpu8)

                        # 로그 기록
                        logging.info("######################################################################")
                        logging.info(f"train/loss: {avg_losses}, global_step: {global_step+gpu4_gpu8}")
                        logging.info(f"train/loss_audio: {avg_losses_audio}, global_step: {global_step+gpu4_gpu8}")
                        logging.info(f"train/loss_video: {avg_losses_video}, global_step: {global_step+gpu4_gpu8}")

                        logging.info(f"train/loss_ta_rc_distill_audio: {avg_losses_rc_distill_audio}, global_step: {global_step+gpu4_gpu8}")
                        logging.info(f"train/loss_tv_rc_distill_video: {avg_losses_rc_distill_video}, global_step: {global_step+gpu4_gpu8}")
                ...
                if (global_step+gpu4_gpu8+1) % (args.eval_every * args.gradient_accumulation) == 0:
                    ckpt_dir = os.path.join(args.ckpt_save_path, f"checkpint_{args.tensorboard_log_dir}/checkpoint-step-{global_step+gpu4_gpu8}")
                    # save checkpoint
                    if accelerator.is_main_process:
                        accelerator.save_state(ckpt_dir)

                        training_state = {"global_step": global_step+gpu4_gpu8, "epoch": epoch, "step": global_step+gpu4_gpu8}
                        with open(os.path.join(ckpt_dir, "training_state.json"), "w") as f:
                            json.dump(training_state, f)
                        print(f"[Step {global_step+gpu4_gpu8}] Checkpoint saved at: {ckpt_dir}")
                    accelerator.wait_for_everyone()
