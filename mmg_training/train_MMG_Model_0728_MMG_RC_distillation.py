...
from dataset_mmg_new_rc_0728 import AudioVideoDataset
...
from MMG_multi_gpu_inference_mmg_0606 import run_inference
...
def parse_args():
    ...
    parser.add_argument('--video_loss_weight', type=float, default=1.0, help='Loss weight for the TAV video component.')
    parser.add_argument('--ta_audio_loss_weight', type=float, default=1.0, help='Loss weight for the TA audio component.')
    parser.add_argument('--tv_video_loss_weight', type=float, default=1.0, help='Loss weight for the TV video component.')
    parser.add_argument('--rc_ta_audio_loss_weight', type=float, default=1.0, help='Loss weight for the TA audio component.')
    parser.add_argument('--rc_tv_video_loss_weight', type=float, default=1.0, help='Loss weight for the TV video component.')
    ...
    parser.add_argument('--openvid_csv_path', type=str, default='/workspace/processed_vggsound_sparse_0218/avsync_test', help='Directory where outputs will be saved.')
    parser.add_argument('--openvid_gt_test_path', type=str, default='/workspace/processed_vggsound_sparse_0218/avsync_gt_test.csv', help='Directory where outputs will be saved.')
...
def linear_biased_timesteps(batch_size, num_timesteps, device):
    probs = torch.arange(1, num_timesteps + 1, dtype=torch.float32, device=device)
    ...
    try:
        with open(prompt_file, newline='', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                ...
                audio_caption = row.get('caption', '').strip()
                video_caption = row.get('new_caption', '').strip()
                caption = (audio_caption, video_caption)
                if caption:
...
def evaluate_model(args, accelerator, target_csv_files, eval_id, target_path, ckpt_dir, modality="all"):
    ...
    # MMG inference
    run_inference(args, accelerator, prompt_sublist, inference_save_path, ckpt_dir, target_csv_files)
...
def main():
    ...
    video_model.load_state_dict(state_dict, strict=True) # 로라 때문에 false
    ...
    model, teacher_audio_unet, teacher_video_unet= model.to(dtype=dtype), teacher_audio_unet.to(dtype=dtype), teacher_video_unet.to(dtype=dtype)
    ...
    # # =================================================
    gpu4_gpu8_step = 0
    # gpu4_gpu8_step = global_step // 2
    # global_step -= gpu4_gpu8_step
    ...
    losses_rc_distill_audio = []
    losses_rc_distill_video = []
        ...
        with tqdm(dataloader, desc=f"Epoch {epoch+1}/{args.num_epochs}", unit="batch") as tepoch:
            ...
            for batch_idx, batch in enumerate(tepoch):
                ...
                with accelerator.accumulate(model):
                    ...
                    spec_tensors = batch["spec"].to(device=device,dtype=dtype, non_blocking=True)
                    video_tensors = batch["video_tensor"].to(device=device,dtype=dtype, non_blocking=True)
                    audio_caption = batch["tav_audio_caption"]
                    video_caption = batch["tav_video_caption"]

                    ta_spec_tensors = batch["ta_spec"].to(device=device,dtype=dtype, non_blocking=True)
                    # ta_caption_list = batch["ta_caption"]
                    ta_audio_caption_list = batch["ta_audio_caption"]
                    ta_video_caption_list = batch["ta_video_caption"]
                    
                    tv_video_tensors = batch["tv_video_tensor"].to(device=device,dtype=dtype, non_blocking=True)
                    # tv_caption_list = batch["tv_caption"]
                    tv_audio_caption_list = batch["tv_audio_caption"]
                    tv_video_caption_list = batch["tv_video_caption"]
                    ...
                    with torch.no_grad():
                        ...
                        with accelerator.autocast():
                            ...
                            tv_video_text_embed = video_model.get_learned_conditioning(tv_video_caption_list)
                            ...
                            # ta_video encoding
                            ta_video_text_embed = video_model.get_learned_conditioning(ta_video_caption_list)
                            tv_video_null_prompts = batch_size * [""]
                            tv_video_null_text_embed = video_model.get_learned_conditioning(tv_video_null_prompts)
                            ta_video_text_embed = torch.where(mask, tv_video_null_text_embed, ta_video_text_embed)
                            tv_fps_tensor = torch.full((batch_size,), args.fps, device=device)
                        ...
                        with accelerator.autocast():
                            ...
                            # tv_audio
                            tv_audio_text_embed = encode_audio_prompt(
                                text_encoder_list=audio_text_encoder_list,
                                tokenizer_list=audio_tokenizer_list,
                                adapter_list=audio_adapter_list,
                                tokenizer_model_max_length=77,
                                dtype=ta_spec_tensors.dtype,
                                prompt=tv_audio_caption_list,
                                device=accelerator.device
                            )
                            tv_audio_null_text_emb = torch.zeros_like(tv_audio_text_embed)  # [batch_size, 77, 768]
                            tv_audio_text_embed = torch.where(mask, tv_audio_null_text_emb, tv_audio_text_embed)  # [batch_size, 77, 768]
                    ...
                    # rc_timesteps = linear_biased_timesteps(batch_size, noise_scheduler.num_train_timesteps, device)

                    ta_audio_output, _ = model(
                        ...
                        video_timestep=timesteps,
                        video_context=ta_video_text_embed,
                        video_fps=tv_fps_tensor
                    )

                    _, tv_video_output = model(
                        audio_latents=ta_noised_audio_latents,
                        audio_timestep=timesteps,
                        audio_encoder_hidden_states=tv_audio_text_embed,
                        video_latents=tv_noised_video_latents,
                        video_timestep=timesteps,
                    ...
                    with torch.no_grad():
                        ta_teacher_audio_output = teacher_audio_unet(
                        ...
                        tv_teacher_video_output = teacher_video_unet(
                    ...
                    mask = (timesteps >= 900).float()  # e.g. torch.Tensor([0,1,1,0, ...])


                    # -- Audio distill loss --
                    # 1) element-wise MSE, reduction='none'으로 batch 차원 보존
                    ta_per_sample = F.mse_loss(
                        ta_audio_output,
                        ta_teacher_audio_output,
                        reduction='none'
                    )
                    # 2) feature 차원을 모두 평균내서 (batch,) 형태로
                    ta_per_sample = ta_per_sample.reshape(ta_per_sample.size(0), -1).mean(dim=1)


                    # 3) mask로 걸러서 평균 (mask.sum()==0 방어)
                    if mask.sum() > 0:
                        ta_loss = (ta_per_sample * mask).sum() / mask.sum()
                    else:
                        ta_loss = torch.tensor(0., device=ta_per_sample.device)

                    rc_ta_distill_loss_audio = args.rc_ta_audio_loss_weight * ta_loss

                    # -- Video distill loss --
                    tv_per_sample = F.mse_loss(
                        tv_video_output,
                        tv_teacher_video_output,
                        reduction='none'
                    )
                    tv_per_sample = tv_per_sample.reshape(tv_per_sample.size(0), -1).mean(dim=1)


                    if mask.sum() > 0:
                        tv_loss = (tv_per_sample * mask).sum() / mask.sum()
                    else:
                        tv_loss = torch.tensor(0., device=tv_per_sample.device)

                    rc_tv_distill_loss_video = args.rc_tv_video_loss_weight * tv_loss


                    # rc distill loss
                    rc_ta_distill_loss_audio = args.rc_ta_audio_loss_weight * F.mse_loss(ta_audio_output, ta_teacher_audio_output)
                    rc_tv_distill_loss_video = args.rc_tv_video_loss_weight * F.mse_loss(tv_video_output, tv_teacher_video_output)


                    loss = loss_audio + loss_video + rc_ta_distill_loss_audio + rc_tv_distill_loss_video
                    ...
                    losses_rc_distill_audio.append(rc_ta_distill_loss_audio.item())
                    losses_rc_distill_video.append(rc_tv_distill_loss_video.item())
                    ...
                    if accelerator.is_main_process:
                        ...
                        avg_losses_rc_distill_audio = sum(losses_rc_distill_audio) / len(losses_rc_distill_audio)
                        avg_losses_rc_distill_video = sum(losses_rc_distill_video) / len(losses_rc_distill_video)
                        
                        # 텐서보드에 로그
                        if writer is not None:
                            writer.add_scalar("train/loss", avg_losses, global_step + gpu4_gpu8_step)
                            writer.add_scalar("train/loss_audio", avg_losses_audio, global_step + gpu4_gpu8_step)
                            writer.add_scalar("train/loss_video", avg_losses_video, global_step + gpu4_gpu8_step)
                            writer.add_scalar("train/loss_ta_rc_distill_audio", avg_losses_rc_distill_audio, global_step + gpu4_gpu8_step)
                            writer.add_scalar("train/loss_tv_rc_distill_video", avg_losses_rc_distill_video, global_step + gpu4_gpu8_step)
                            writer.add_scalar("epoch", epoch, global_step + gpu4_gpu8_step)

                        # 로그 기록
                        logging.info("######################################################################")
                        logging.info(f"train/loss: {avg_losses}, global_step: {global_step + gpu4_gpu8_step}")
                        logging.info(f"train/loss_audio: {avg_losses_audio}, global_step: {global_step + gpu4_gpu8_step}")
                        logging.info(f"train/loss_video: {avg_losses_video}, global_step: {global_step + gpu4_gpu8_step}")

                        logging.info(f"train/loss_ta_rc_distill_audio: {avg_losses_rc_distill_audio}, global_step: {global_step + gpu4_gpu8_step}")
                        logging.info(f"train/loss_tv_rc_distill_video: {avg_losses_rc_distill_video}, global_step: {global_step + gpu4_gpu8_step}")
                        ...
                        wandb.log({
                            ...
                            "train/loss_ta_rc_distill_audio": avg_losses_rc_distill_audio,
                            "train/loss_tv_rc_distill_video": avg_losses_rc_distill_video,                        
                            "epoch": epoch,
                            "step": global_step + gpu4_gpu8_step
                        })
                ...
                if (global_step+1) % (args.eval_every * args.gradient_accumulation) == 0:
                    ckpt_dir = os.path.join(args.ckpt_save_path, f"checkpint_{args.tensorboard_log_dir}/checkpoint-step-{global_step + gpu4_gpu8_step}")
                    # save checkpoint
                    if accelerator.is_main_process:
                        accelerator.save_state(ckpt_dir)

                        training_state = {"global_step": global_step + gpu4_gpu8_step, "epoch": epoch, "step": global_step + gpu4_gpu8_step}
                        with open(os.path.join(ckpt_dir, "training_state.json"), "w") as f:
                            json.dump(training_state, f)
                        print(f"[Step {global_step + gpu4_gpu8_step}] Checkpoint saved at: {ckpt_dir}")
                    accelerator.wait_for_everyone()
                    ...
                    # evaluate model
                    openvid_fad, openvid_clap_avg, openvid_fvd, openvid_clip_avg, openvid_imagebind_score, openvid_av_align = evaluate_model(
                        args=args,
                        accelerator=accelerator,
                        target_csv_files=args.openvid_csv_path,
                        target_path=args.openvid_gt_test_path,
                        eval_id=f"{args.infer_name}_step_{global_step}_openvid",
                        ckpt_dir=safetensor_path
                    )
                    accelerator.wait_for_everyone()
                    ...
                    ac_fad, ac_clap_avg, ac_fvd, ac_clip_avg, ac_imagebind_score, ac_av_align = evaluate_model(
                        ...
                        eval_id=f"{args.infer_name}_step_{global_step}_audiocaps",
                        ckpt_dir=safetensor_path
                    )
                    accelerator.wait_for_everyone()
