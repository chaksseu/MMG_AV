...
def evaluate_audio_metrics(preds_folder, target_folder, metrics, clap_model, device, csv_path=None):
    fad_score = -1
    ...
    for filename in progress_bar:
        if filename.endswith('.wav') or filename.endswith('.flac'):
            try:
                ...
                clap_score.append(calculate_clap(model_clap, preds_audio, filename, new_freq, csv_path))
