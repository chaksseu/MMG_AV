...
def evaluate_av_align_score(audio_inference_path, video_inference_path):

    files = [file for file in glob.glob(os.path.join(audio_inference_path, '*.wav'))]
    score = 0
    cnt = 0
    ...
    for file in tqdm(files, desc="Processing files for av-align"):
        ...
        video_path = os.path.join(video_inference_path, f'{file}.mp4')
        audio_path = os.path.join(audio_inference_path, f'{file}.wav')
