...
def load_videos_with_caps(folder_path, num_frames, csv_path):
    videos_tensor_list_orig = []
    caps = []
    ...
    for video_name in vid_fnames:
        # fname = video_name[:-4]
        fname = video_name.split('_batch_')[0]
        # print(fname)
        video_caption = get_new_caption_from_csv_0603(fname, csv_path)
...
def evaluate_video_metrics(preds_folder, target_folder, metrics, device, num_frames, csv_path):
    ...
    if 'clip' in metrics:
        clip_videos, caps = load_videos_with_caps(preds_folder, num_frames, csv_path)
        clip_score = calculate_clip(clip_videos, caps, calculate_per_frame, calculate_final, '/home/work/kby_hgh/openai_clip_local', device)
...
def main():
    ...
    parser.add_argument(
        "--csv_path",
        type=str,
        default="/home/work/kby_hgh/vggsound_sparse_test_curated_final_0320/vggsound_sparse_curated_292.csv",
        help="예측 비디오 파일들이 있는 폴더 경로"
    )
    ...
    for checkpoint in checkpoint_dir_list:
        args.metrics = ["fvd", "clip"]
        args.device = "cuda:0"


        args.target_folder = f"/home/work/kby_hgh/vggsound_sparse_test_curated_final_0320/video"
        args.preds_folder = f"/home/work/kby_hgh/0603_video_teacher_LORA_vggsound_sparse_inference/checkpoint-step-{checkpoint}"
