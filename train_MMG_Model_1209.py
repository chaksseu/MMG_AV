...
# 이전에 작성한 dataset, dataloader를 그대로 가져온다고 가정
   ...
   #     print("Processing time embeddings")
    ...
    #    print("Processing encoder hidden states")
     ...
     #   print("  [audio_down_block] 0")
        ...
        #print("cross_video_latent_token", cross_video_latent_token.shape)
        #print("cross_audio_latent_token", cross_audio_latent_token.shape)
...
def main():
    ...
    # Optimizer 정의
    # 모델 자체의 파라미터는 cross_modal_transformer 부분 외에 freeze되어있으므로, 업데이트 가능한 파라미터만 optimizer에 넣음
    # 여기서는 예시로 model.parameters() 넣었으나, 실제 학습 시에는 trainable한 파라미터만 필터링 필요할 수 있음
        ...
        with tqdm(dataloader, desc=f"Epoch {epoch+1}/{num_epochs}", unit="batch") as tepoch:
            for batch_idx, batch in enumerate(tepoch):
                ...
                if torch.rand(1).item() < 0.1:
                    audio_text_embed = audio_null_text_embed
                    video_text_embed = video_null_text_embed
                ...
                loss = audio_weight * loss_audio + video_weight * loss_video
                ...
                if accelerator.is_main_process and global_step % 10000 == 0:
                    checkpoint_path = f"mmg_checkpoints/global_step_{global_step}_batch_{full_batch_size}"
