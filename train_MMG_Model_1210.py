...
class CrossModalCoupledUNet(nn.Module):
    def __init__(self, audio_unet, video_unet, cross_modal_config):
        ...
        #channel = layer_channels[0]
        #self.audio_adaptation_in.append(nn.Linear(channel, channel))
        #self.audio_adaptation_out.append(nn.Linear(channel, channel))
        #self.video_adaptation_in.append(nn.Linear(channel, channel))
        #self.video_adaptation_out.append(nn.Linear(channel, channel))
  ...
  #      print(f"    [blocks]: {layer_type}")
    ...
    def linear_cmt(self, audio_hidden_states, h, index):
        ...
        # --- Linear In ---
        condition_cross_video_latent_token = rearrange(cross_video_latent_token, 'b c n -> (b n) c')
        ...
        condition_cross_audio_latent_token = rearrange(cross_audio_latent_token, 'bt c f -> (bt f) c')
   ...
   #     print("Processing time embeddings")
    ...
    #    print("Processing encoder hidden states")
     ...
     #   print("  [audio_down_block] 0")
        ...
        # --- Linear In ---
        cross_video_latent_token = rearrange(cross_video_latent_token, 'b c n -> (b n) c')
        cross_video_latent_token = self.video_adaptation_in[0](cross_video_latent_token)
        cross_video_latent_token = rearrange(cross_video_latent_token, '(b n) c -> b c n', b=b_a * t_a)
        cross_audio_latent_token = rearrange(cross_audio_latent_token, 'bt c f -> (bt f) c')
        cross_audio_latent_token = self.audio_adaptation_in[0](cross_audio_latent_token)
        cross_audio_latent_token = rearrange(cross_audio_latent_token, '(bt f) c -> bt c f', bt=b_a*t_a)
        ...
        # --- Linear Out ---
        cross_video_latent_token = rearrange(cross_video_latent_token, 'b c n -> (b n) c')
        cross_video_latent_token = self.video_adaptation_out[0](cross_video_latent_token)
        cross_video_latent_token = rearrange(cross_video_latent_token, '(b n) c -> b c n', b=b_a * t_a)
        cross_audio_latent_token = rearrange(cross_audio_latent_token, 'bt c f -> (bt f) c')
        cross_audio_latent_token = self.audio_adaptation_out[0](cross_audio_latent_token)
        cross_audio_latent_token = rearrange(cross_audio_latent_token, '(bt f) c -> bt c f', bt=b_a*t_a)
...
def main():
    ...
    noise_scheduler = PNDMScheduler.from_pretrained("auffusion/auffusion-full", subfolder="scheduler")
