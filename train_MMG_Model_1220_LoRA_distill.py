...
def main():
    ...
    ###audio_distill_loss_weight, video_distill_loss_weight = 1.0, 4.0
    ...
    '''
    audio_teacher_unet = UNet2DConditionModel.from_pretrained(audio_model_name, subfolder="unet")
    audio_teacher_unet.eval()
    for param in audio_teacher_unet.parameters():
        param.requires_grad = False
    '''
    ...
    '''
    video_teacher_model = instantiate_from_config(video_config.model)
    video_teacherstate_dict = torch.load(videocrafter_ckpt)['state_dict']
    video_teacher_model.load_state_dict(video_teacherstate_dict, strict=False) # 로라 때문에 false
    ...
    for epoch in range(num_epochs):
        with tqdm(dataloader, desc=f"Epoch {epoch+1}/{num_epochs}", unit="batch") as tepoch:
            for batch_idx, batch in enumerate(tepoch):
                ...
                '''
                ################# audio teacher unet forward #####################
                audio_teacher_output = audio_teacher_unet(
                ...
                ############## video teacher unet forward #################
                video_teacher_output = video_teacher_unet(
                ...
                ###audio_distill_loss = audio_distill_loss_weight * F.mse_loss(audio_output, audio_teacher_output)
                ###video_distill_loss = video_distill_loss_weight * F.mse_loss(video_output, video_teacher_output)


                loss = loss_audio + loss_video ###+ audio_distill_loss + video_distill_loss
