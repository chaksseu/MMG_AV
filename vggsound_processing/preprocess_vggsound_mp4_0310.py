...
# 설정 변수
SOURCE_DIR = '/workspace/data/vggsound_train_0313'  # 원본 mp4 파일이 있는 폴더 경로
DEST_DIR = '/workspace/data/preprocessed_VGGSound_train_videos_0310'  # 전처리된 파일을 저장할 폴더 경로
MIN_DURATION = 4        # 최소 길이 (초)
MAX_DURATION = 30       # 최대 길이 (초)
RESIZE_WIDTH = 512
RESIZE_HEIGHT = 320
TARGET_FRAME_COUNT = 125  # 최종 출력 영상이 가지게 될 프레임 수
OUTPUT_FPS = 12.5           # VideoWriter를 위한 형식상 fps (실제 프레임 수는 TARGET_FRAME_COUNT로 고정)
num_workers = 16
...
def resize_video_fixed_frames(input_path, output_path, width, height, target_frame_count):
    ...
    width x height로 리사이즈 후 새 MP4로 저장.
    """
    ...
    # (형식상) 출력 동영상의 fps (실제 영상 길이에 관계없이 총 프레임 수를 맞추기 위한 것이므로
    # 여기서는 30으로 설정. 필요에 따라 다른 값으로 바꾸셔도 됩니다.)
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out = cv2.VideoWriter(output_path, fourcc, OUTPUT_FPS, (width, height))

    # 만약 원본 프레임 수가 목표 프레임 수보다 작다면
    # 그대로 모든 프레임을 쓰거나, 스킵할 것인지 정책 결정이 필요합니다.
    # 예시에서는 "모든 프레임을 쓰되 부족하면 영상을 그대로 짧게 유지"로 가정.
