...
def main(args):
    ...
    # video_unet = video_model.model.diffusion_model
    ...
    # 특정 부분만 학습
    for name, param in video_model.model.diffusion_model.named_parameters():
    ...
    # 전체 파라미터 개수 계산
    total_params = sum(p.numel() for p in video_model.model.diffusion_model.parameters())
    # 학습 가능한 파라미터 개수 계산
    trainable_params = sum(p.numel() for p in video_model.model.diffusion_model.parameters() if p.requires_grad)
    ...
    # 학습 가능한 파라미터 리스트 생성
    trainable_params_list = [p for p in video_model.model.diffusion_model.parameters() if p.requires_grad]
    optimizer = torch.optim.AdamW(trainable_params_list, lr=args.lr)


    # prepare
    video_model.model.diffusion_model, optimizer, train_loader = accelerator.prepare(
        video_model.model.diffusion_model, optimizer, train_loader
    )
    ...
    writer = None
    if accelerator.is_main_process:
        writer = SummaryWriter(log_dir="/home/work/kby_hgh/MMG_01/tensorboard_video_lora_0412")

    video_model.model.diffusion_model.train()
    ...
    for epoch in range(args.num_epochs):
        ...
        #         accelerator=accelerator,
        #         unet_model=video_model.model.diffusion_model,
        #         video_model=video_model,
        ...
        for step, batch in enumerate(loop):
            ...
            with accelerator.accumulate(video_model.model.diffusion_model):
                optimizer.zero_grad()

                for name, param in video_model.model.diffusion_model.named_parameters():
                    if param.requires_grad:
                        param_before = param.detach().cpu().clone()
                        param_name = name
                        param_ref = param  # 원본 포인터
                        break
                ...
                # UNet forward
                video_original_output = video_model.model.diffusion_model(
                    noised_video_latent,
                    timesteps,
                ...
                # accelerator.clip_grad_norm_(video_model.model.diffusion_model.parameters(), max_norm=1.0)
###
...
###
                ...
                param_after = param_ref.detach().cpu()

                if accelerator.is_main_process:
                    if not torch.equal(param_before, param_after):
                        print(f">>> Parameter [{param_name}] was updated!")
                    else:
                        print('############################################')
                        print('############################################')
                        print('############################################')
                        print('############################################')
                        print('############################################')
                        print(f">>> Parameter [{param_name}] unchanged.")
